...
class _DnaSettingScreenState extends State<DnaSettingScreen> with SingleTickerProviderStateMixin {
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: BlocBuilder<DnaCubit, DnaState>(
                            buildWhen: (previous, current) => current is DnaInitial || current is DnaUpdateSetNumber,
                        ...
                        Container(
                          margin: EdgeInsets.only(left: size*0.05, right: size*0.07,top: size*0.02),
                          child: BlocBuilder<DnaCubit, DnaState>(
                            builder: (context, state) {
                              return Row(
                                ...
                                children: [
                                  RadioBoxButton(
                                    ...
                                    onChanged: (value) {
                                      ...
                                      context.read<DnaCubit>().updateBreathingApproach(value!);
                                    },
                                  ),
                                  ...
                                  RadioBoxButton(
                                    ...
                                    value: "Time per set", 
                                    onChanged: (value) {
                                      context.read<DnaCubit>().isTimeBreathingApproch = true;
                                      context.read<DnaCubit>().updateBreathingApproach(value!);
                                    },
                                  ),
                                ],
                          ...
                          builder: (context, state) {
                            if(context.read<DnaCubit>().isTimeBreathingApproch){
                              return Container(
                                ...
                                margin: EdgeInsets.only(left: size*0.05, right: size*0.07),
                                child: BlocBuilder<DnaCubit, DnaState>(
                                  buildWhen: (previous, current) => current is DnaInitial || current is DnaUpdateBreathTime,
                                  builder: (context, state) {
                                    return SettingsDropdownButton(
                                      onSelected: (int selectedBreath) {
                                        context.read<DnaCubit>().updateBreathTime(selectedBreath);
                                      },
                                      title: "Time per set:",
                                      selected: context.read<DnaCubit>().durationOfSet, 
                                      options: context.read<DnaCubit>().durationsList, 
                                      isTime: true,
                                    );
                                  }, 
                                ),
                            ...
                            return Container(
                              ...
                              margin: EdgeInsets.only(left: size*0.05, right: size*0.07),
                              child: BlocBuilder<DnaCubit, DnaState>(
                                buildWhen: (previous, current) => current is DnaInitial || current is DnaUpdateBreathNumber,
                                builder: (context, state) {
                                  return SettingsDropdownButton(
                                    onSelected: (int selectedBreath) {
                                      context.read<DnaCubit>().updateBreathNumber(selectedBreath);
                                    },
                        ...
                        Container(
                          ...
                          margin: EdgeInsets.symmetric(horizontal: size*0.05),
                          child: BlocBuilder<DnaCubit, DnaState>(
                            buildWhen: (previous, current) => current is DnaInitial || current is DnaToggleRecoveryBreath,
                            builder: (context, state) {
                              return SettingsToggleButton(
                                onToggle: () {
                                  context.read<DnaCubit>().toggleRecoveryBreath();
                                }, 
                        ...
                        Container(
                          ...
                          margin: EdgeInsets.symmetric(horizontal: size*0.05),
                          child: BlocBuilder<DnaCubit, DnaState>(
                            buildWhen: (previous, current) => current is DnaInitial || current is DnaToggleJerryVoice,
                            builder: (context, state) {
                              return SettingsToggleButton(
                                onToggle: () {
                        ...
                        Container(
                          ...
                          margin: EdgeInsets.symmetric(horizontal: size*0.05),
                          child: BlocBuilder<DnaCubit, DnaState>(
                            buildWhen: (previous, current) => current is DnaInitial || current is DnaToggleMusic,
                            builder: (context, state) {
                              return SettingsToggleButton(
                                onToggle: () {
                        ...
                        Container(
                          ...
                          margin: EdgeInsets.symmetric(horizontal: size*0.05),
                          child: BlocBuilder<DnaCubit, DnaState>(
                            buildWhen: (previous, current) => current is DnaInitial || current is DnaToggleChimes,
                            builder: (context, state) {
                              return SettingsToggleButton(
                                onToggle: () {
          ...
          Expanded(
            child: GestureDetector(
              onTap: () {
                ...
                context.read<DnaCubit>().speed = "Slow" ;
              },
              child: Container(
                alignment: Alignment.center,
                width: size,
          ...
          Expanded(
            child: GestureDetector(
              onTap: () {
                ...
                context.read<DnaCubit>().speed = "Standard" ;
              },
              child: Container(
                alignment: Alignment.center,
                width: size,
          ...
          Expanded(
            child: GestureDetector(
              onTap: () {
                ...
                context.read<DnaCubit>().speed = "Fast" ;
              },
              child: Container(
                alignment: Alignment.center,
                width: size,
